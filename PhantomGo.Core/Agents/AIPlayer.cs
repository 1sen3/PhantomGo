...
namespace PhantomGo.Core.Agents
{
    public class AIPlayer : IPlayerAgent, IDisposable
    {
        ...
        public AIPlayer(int boardSize, Player playerColor, int simulationPerMove = 800)
        {
        ...
        private List<Point> FindInnerEyes(GoBoard selfBoard)
        {
            ...
            for (int x = 1; x <= _boardSize; x++)
            {
                for (int y = 1; y <= _boardSize; y++)
                {
                    ...
                    {
                        if (j.X >= 1 && j.X <= _boardSize && j.Y >= 1 && j.Y <= _boardSize)
                        {
        ...
        {
            return (p.Y - 1) * _boardSize + (p.X - 1);
        }

        /// <summary>
        /// 将扁平索引 (0-based) 转换为 (x, y) 坐标 (1-based)
        /// </summary>
        private Point IndexToPoint(int index)
        {
            int y = (index / _boardSize) + 1;
            int x = (index % _boardSize) + 1;
